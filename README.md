...
```bash
sudo apt-get update
sudo apt-get install -y fontconfig libfontconfig1 libfontconfig1-dev fonts-noto-cjk
```

4. 字体配置
- 优先使用 `assets` 目录中的思源黑体文件（如果存在）
- 如果没有，系统会自动查找并使用：
  - Linux: 思源黑体或文泉驿微米黑
  - Windows: 思源黑体或微软雅黑
  - macOS: 思源黑体或苹方字体
...
├── assets/              # 资源文件目录
│   └── SourceHanSansCN-Regular.otf  # 思源黑体字体文件（可选）
...
1. 系统会自动安装 `requirements.txt` 中的Python依赖
2. 系统会自动安装 `packages.txt` 中的系统依赖
3. 字体配置会自动完成，按优先级使用：
   - assets目录中的思源黑体
   - 系统安装的思源黑体
   - 系统默认中文字体
